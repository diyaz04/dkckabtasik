...
const targetString = "<GreetingBanner name={user?.nama || 'Administrator'} role={user?.role} />";
content = content.replace(targetString, desktopHeader + '\n          ' + targetString);

// BUT since we are duplicating the dropdowns, we need to hide the mobile ones on desktop to avoid weird bugs?
// The mobile ones are inside the `<div className="md:hidden ...">`, so they are automatically hidden on desktop by `md:hidden`!
// So it's safe to just inject this for desktop because it's wrapped in `<div className="hidden md:flex ...">` and the other is wrapped in `md:hidden`.

fs.writeFileSync('src/components/PortalAdmin.tsx', content);
console.log('Added desktop header for PortalAdmin');
